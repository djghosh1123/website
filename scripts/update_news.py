...
SEARCH_QUERIES = [
    '"Dhrubajyoti Ghosh" "Kennesaw State University" 2026 publication OR conference OR invited OR award',
    '"Dhrubajyoti Ghosh" Kennesaw statistics 2026 site:kennesaw.edu OR site:amstat.org OR site:arxiv.org',
    '"Dhrubajyoti Ghosh" 2026 CRAN OR IEEE OR Springer OR Wiley OR SAGE OR SciTePress',
    '"Dhrubajyoti Ghosh" student research Kennesaw 2026'
]
...
def serp_search(api_key):
    ...
    for query in SEARCH_QUERIES:
        ...
        for r in payload.get("organic_results", []):
            ...
            # Prefer trusted academic/institutional sources, but keep strong exact-name results.
...
def read_cv_context():
    ...
    try:
        ...
        return ""
    # Redact lines with provisional statuses before the text ever reaches the LLM.
    safe_lines = []
...
Your job is NOT to write a general web summary. Select only genuinely newsworthy NEW public updates about this specific statistician/researcher. Reject homonyms. Strong identity signals include Kennesaw State University, Duke Biostatistics collaborations, Washington University statistics collaborators, Sheng Luo, Soumendra Lahiri, Tucker McElroy, Samhita Pal, or the research areas above.

Allowed categories: Publication, Student research, Invited talk, Conference, Software, Award, Lab news, Funding (only if a grant/award is publicly confirmed as funded).
...
- Keep each summary factual, compact, and suitable for an academic website. No hype.
...
{{"date":"YYYY-MM-DD","date_display":"Month YYYY","category":"...","title":"...","summary":"2-3 factual sentences","source_url":"exact candidate URL","source_label":"short source name","featured":false}}
Use conservative dates. If the exact day is unsupported, use the first day of the supported month and make date_display just Month YYYY. Do not add an item just because a page mentions his name.
"""
    body = json.dumps({
        "model": MODEL,
        "input": prompt,
...
def validate_items(items, candidates, news):
    ...
    for item in items if isinstance(items, list) else []:
        ...
        if title_fp in existing or url_fp in existing:
            continue
        if item["category"] not in {"Publication","Student research","Invited talk","Conference","Software","Award","Lab news","Funding"}:
            continue
